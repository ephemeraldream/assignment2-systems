...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, x: Float[Tensor, " ... seq d"], pos_ids: Int[Tensor, " ... seq"]) -> Float[Tensor, " ... seq d"]:
        ...
        # Use standard PyTorch advanced indexing instead of einx.get_at for better torch.compile stability
        # self._freq_cis_cache shape: (2, context_length, half_dim)
        # pos_ids shape: (..., seq)
        # Resulting cos/sin shapes: (..., seq, half_dim)
        gathered = self._freq_cis_cache[:, pos_ids, :]
        cos = gathered[0]
        sin = gathered[1]
        ...
        result = torch.cat([x1_rot, x2_rot], dim=-1).contiguous()
        return result
    ...
    def forward(self, x: Float[Tensor, " ... seq d_k"], token_positions: Int[Tensor, " ... seq"] | None = None) -> Float[Tensor, " ... seq d_v"]:
        ...
        *b, sequence_length, d_model = x.size()
        ...
        if token_positions is None:
            ...
            # reshape to (*[1]*len(b), seq) and expand to (*b, seq)
            token_positions = token_positions.view(*([1] * len(b)), sequence_length).expand(*b, sequence_length)
        ...
        # Build broadcastable causal mask of shape (*b, 1, seq, seq)
        qi = seq.view(*([1] * len(b)), 1, sequence_length, 1)
        kj = seq.view(*([1] * len(b)), 1, 1, sequence_length)
